...
function detalle_compra(CompraNo)
{
    $.ajax({

       type:"GET",
       url:"web/ajax/reload-detalle-compra.php?numero_transaccion="+CompraNo,
        ...
        function(isConfirm){
            if (isConfirm) {
                 return new Promise(function(resolve) {
                    $.ajax({
                    url: 'web/ajax/ajxcreditoproveedor.php',
                    type: 'POST',
                    ...
                    .done(function(response){
                     ...
                     cargarDiv("#reload-div","web/ajax/reload-creditoproveedor.php");
                    })
                    .fail(function(){
...
function Print_Report(Criterio)
{
    ...
    {
         window.open('reportes/Creditos_Vigentes_Proveedor.php?',
        'win2',
...
function Limite_Abono()
{
    ...
    if(credito!=null){
      ...
      $.getJSON('web/ajax/ajxcreditoproveedor.php?credito='+credito,function(data){
...
{
   window.open('reportes/Ticket_Abono_V_Proveedor.php?abono='+btoa(Abono),
...
{
   window.open('reportes/Credito_Detalle_Proveedor.php?cod='+btoa(Cod),
  'win2','status=yes,toolbar=yes,scrollbars=yes,titlebar=yes,menubar=yes,'+
  'resizable=yes,width=600,height=600,directories=no,location=no'+
  'fullscreen=yes');
...
{
  var urlprocess = 'web/ajax/ajxcreditoproveedor.php';
  var opcion = 'ABONO';
  var id = $("#txtID").val();
  ...
  var dataString='proceso='+proceso+'&fecha_abono='+fecha_abono+'&monto_abono='+monto_abono+'&idcreditoproveedor='+idcredito+'&opcion='+opcion+'&id='+id;
...
function imprimir_abono_reporte(){
  ...
  var fecha2 = $("#txtHasta").val();
    ...
    if(fecha1!="" && fecha2!="")
    {
        window.open('reportes/Total_Abonos_Proveedor.php?fecha1='+fecha1+'&fecha2='+fecha2,
       'win2',
       'status=yes,toolbar=yes,scrollbars=yes,titlebar=yes,menubar=yes,'+
       'resizable=yes,width=800,height=800,directories=no,location=no'+
       'fullscreen=yes');
...
function Print_Ticket(Abono){
    var url = 'reportes/Ticket_Abono_O_Proveedor.php?abono='+btoa(Abono);
    //var url2 = 'reportes/Ticket_Abono_C_Proveedor.php?abono='+btoa(Abono);
